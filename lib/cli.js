...
async function run(argv) {
  ...
  function getConfig() {
    return getConfigContext().resolved
  }
  ...
  switch (command) {
    ...
    case "me": {
      const me = await clickupRequest(getToken(), "/user")
      printJson({
        id: me.user.id,
    ...
    case "teams": {
      const result = await clickupRequest(getToken(), "/team")
    ...
    case "spaces": {
      ...
      const result = await clickupRequest(getToken(), `/team/${teamId}/space`)
    ...
    case "folders": {
      ...
      const result = await clickupRequest(getToken(), `/space/${spaceId}/folder`)
    ...
    case "space-lists": {
      ...
      const spaceId = args.space || config.spaceId
      assertRequired(spaceId, "Provide a space id with --space or config")
      const result = await clickupRequest(getToken(), `/space/${spaceId}/list`)
    ...
    case "lists": {
      ...
      const result = await clickupRequest(getToken(), `/folder/${folderId}/list`)
      printCollection(result.lists, ["id", "name", "task_count"])
      return
    }
    ...
    case "tasks": {
      ...
      const result = await clickupRequest(getToken(), `/list/${listId}/task${suffix}`)
    ...
    case "task": {
      ...
      const task = await clickupRequest(getToken(), `/task/${taskId}`)
    ...
    case "create-task": {
      ...
      const task = await clickupRequest(getToken(), `/list/${listId}/task`, { method: "POST", body: payload })
      printJson(mapTask(task))
      return
    }
    ...
    case "update-task": {
      ...
      const task = await clickupRequest(getToken(), `/task/${taskId}`, { method: "PUT", body: payload })
      printJson(mapTask(task))
      return
    }
    ...
    case "comment": {
      ...
      const result = await clickupRequest(getToken(), `/task/${taskId}/comment`, {
        method: "POST",
    ...
    case "create-plan": {
      return runPlanCommand(args, getConfigContext(), getToken())
    }
    ...
    case "update": {
      ...
      if (Object.keys(payload).length > 0) {
        await clickupRequest(getToken(), `/task/${taskId}`, { method: "PUT", body: payload })
      }
      ...
      const message = buildProgressComment({
        ...
        files: parseOptionalList(args.files),
        tests: args.tests,
        next: args.next,
      })
      const result = await clickupRequest(getToken(), `/task/${taskId}/comment`, {
    ...
    case "touch": {
      ...
      let commentId = null
      if (args.comment || args.summary || args.files || args.tests || args.next) {
        const message = buildProgressComment({
          prefix: "Update",
          summary: args.comment || args.summary || "Task touched",
          files: parseOptionalList(args.files),
          tests: args.tests,
          next: args.next,
        })
    ...
    case "create-plan-legacy": {
      ...
      const createdSubtasks = await Promise.all((plan.subtasks || []).map(async (subtask) => {
    ...
    case "sync-metadata": {
      ...
      const results = await Promise.all([metadata.parent, ...(metadata.subtasks || [])].map(async (item) => {
    ...
    case "start": {
      ...
      await clickupRequest(getToken(), `/task/${taskId}/comment`, {
        method: "POST",
...
function buildProgressComment({ prefix, summary, commits = [], files = [], tests, next }) {
...
function buildReviewComment({ summary, prUrl, files = [], tests, next, commits = [] }) {
...
function buildDoneComment({ summary, files = [], commits = [], tests, next }) {
...
function getGitContext() {
  const revParse = runGit("git rev-parse --is-inside-work-tree --git-dir --show-toplevel")
  const revParseLines = revParse ? revParse.split("\n") : []
  const insideWorkTree = revParseLines[0] === "true"

  if (!insideWorkTree) {
    return { insideWorkTree: false }
  }

  const gitStatus = runGit("git status --short --branch")
  const gitStatusLines = gitStatus ? gitStatus.split("\n") : []
  const branchLine = gitStatusLines[0] || ""
  const branchMatch = branchLine.match(/^##\s+([^\.\s]+)/)
  const branch = branchMatch && branchMatch[1] !== "HEAD" ? branchMatch[1] : ""
  const statusShort = gitStatusLines.slice(1).join("\n").trim()

  return {
    insideWorkTree: true,
    gitDir: revParseLines[1] || "",
    branch,
    repoName: path.basename(revParseLines[2] || process.cwd()),
...
function printHelp() {
  const helpText = [
    ...
    "  clickup-cli update --task <taskId> --summary \"Key progress\" --files \"src/a.js,src/b.js\" --tests \"passed\" --next \"open PR\"",
    "  clickup-cli review --task <taskId> --files \"src/a.js\" --tests \"npm test\"",
    "  clickup-cli sync --task <taskId>",
    "  clickup-cli link-pr --task <taskId>",
    "  clickup-cli done --task <taskId> --comment \"Finished implementation\" --tests \"passed\"",
