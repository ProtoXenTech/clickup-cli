...
async function run(argv) {
  ...
  switch (command) {
    ...
    case "sync": {
      const taskId = resolveTaskId(args, configContext.state)
      const git = getGitContext()
      const summary = buildSyncSummary(git)
      await clickupRequest(token, `/task/${taskId}/comment`, {
        method: "POST",
        body: { comment_text: summary, notify_all: false },
      })
      printJson({ taskId, synced: true, summary })
      return
    }
...
function resolveTaskId(args, state) {
  ...
  assertRequired(taskId, "Pass --task <taskId> or set an active task with `clickup-dev start --task ...`")
  return taskId
}
...
function buildSyncSummary(git) {
  if (!git.insideWorkTree) {
    return `Sync update from ${path.basename(process.cwd())}: not inside a git repository.`
  }
